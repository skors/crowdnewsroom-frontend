...
export default function ThankYou(props) {
  ...
  const url = window.encodeURIComponent(
    `${origin}/investigation/${investigation}/forms/${form}`
  );
  const message = window.encodeURIComponent(
    "I just contributed to the crowdnewsroom. And you should, too!"
  );

  const urls = {
    facebook: `https://facebook.com/sharer/sharer.php?u=${url}`,
    twitter: `https://twitter.com/intent/tweet/?text=${message}&amp;url=${url}`,
    mail: `mailto:?subject=${message}&amp;body=${url}`,
    telegram: `https://telegram.me/share/url?text=${message}&amp;url=${url}`
  };
